...
if  [ "$dry_run" = "--dry-run" ] ; then 
 ...
 certbot certonly --webroot -d $mydomain --email info@$mydomain -w /var/www/_letsencrypt -n --agree-tos --force-renewal --dry-run
 mkdir -p  /etc/letsencrypt/live/$mydomain
else
 printf "${INFO}Obtaining SSL certificates from Let's Encrypt using Certbot${NC}"
 certbot certonly --webroot -d $mydomain --email info@$mydomain -w /var/www/_letsencrypt -n --agree-tos --force-renewal 
